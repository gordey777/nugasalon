	...
	function mh_newsdesk_lite_content_width() {
		...
		if (!isset($content_width)) {
			if (is_page_template('page-full.php')) {
				$content_width = 960; //old value 1180
			} else {
				$content_width = 777;
			}
		}
...
if (!function_exists('mh_newsdesk_lite_widgets_init')) {
    ...
    register_sidebar(array('name' => esc_html__('Footer - Widgets', 'mh-newsdesk-lite'), 'id' => 'footer-widgets', 'description' => esc_html__('Widgets on Footer', 'mh-newsdesk-lite'), 'before_widget' => '<div id="%1$s" class="footer-widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h4 class="widget-title"><span>', 'after_title' => '</span></h4>'));
	}
}
...
function wpeHeadRightNav() {
  ...
  array(
    ...
    'link_before'     => '',
    'link_after'      => '',
    'items_wrap'      => '<ul class="menu-right right-nav">%3$s</ul>',
    'depth'           => 0,
    'walker'          => ''
    )
  );
...
function register_html5_menu() {
  register_nav_menus(array(
    'header-right-menu' => __('Правое Меню в шапке'),
  ));
}
