	...
	function AdvancedSlider(instance, options) {
		...
		var VERSION = '3.0',
			...
			slideProps = ['htmlDuringTransition', 'alignType', 'effectType', 'sliceDelay', 'sliceDuration', 'sliceEasing', 'horizontalSlices', 'verticalSlices', 'slicePattern', 'slicePoint', 'slideStartPosition',
						  ...
						  'captionBackgroundColor', 'captionBackgroundOpacity', 'slideshowDelay', 'slideMask', 'linkTarget', 'simpleSlideDirection', 'simpleSlideDuration', 'simpleSlideEasing',
						  'lightboxDefaultWidth', 'lightboxDefaultHeight', 'lightboxTheme', 'lightboxOpacity', 'fadePreviousSlide', 'fadePreviousSlideDuration', 'thumbnailType'];
			...
			if (self.settings.xmlSource) {
							...
							$(xml).find('slide').each(function() {
								...
								for (var i = 0; i < $(this).children().length; i++) {
									...
									if (node.nodeName == 'lightboxContent') {
										...
										var lightboxIndex = prettyPhotoContent.length - 1;

										slide.lightbox = {lightbox_index: lightboxIndex,
														  lightbox_content: prettyPhotoContent[lightboxIndex],
														  lightbox_title: prettyPhotoTitle[lightboxIndex],
														  lightbox_description: prettyPhotoDescription[lightboxIndex]};
									}
				...
				slider.children().each(function(index) {
					...
					for (var i = 0; i < $(this).children().length; i++) {
						...
						if($(data).is('a')) {
							if (!$(data).hasClass('slider-lightbox')) {
		...
		*/
		function create() {
			...
			if (self.settings.thumbnailType == 'navigation')
		...
		function createNavigationButtons() {
			...
			for (var i = 0; i < numButtons; i++) {
				...
				button.hover(function() {
								...
								if (slides[index].properties.thumbnailType == 'tooltip')
									showThumbnail(index);
							},
		...
		*/
		function gotoSlide(index) {
			...
			if (self.settings.thumbnailType == 'navigation') {
				...
				navigationThumbnails.find('.thumbnail[rel=' + index + ']').removeClass('over').addClass('select');
		...
		*/
		function startTransition() {
			...
			var	sliceWidth = Math.floor(self.settings.width / horizontalSlices),
				sliceHeight = Math.floor(self.settings.height / verticalSlices),
					...
					for (var j = 0; j < verticalSlices; j++) {
						...
						if (slideData.path) {
							slice.css({'background-image': 'url(' + slideData.path + ')',
									   'background-position': (-i * sliceWidth - offsetBgLeft + offsetPosLeft) + 'px' + ' ' + (-j * sliceHeight - offsetBgTop + offsetPosTop) + 'px',
									   'background-repeat': 'no-repeat'});
						}
		...
		*/
		function startSlideshow() {
			var delay = slides[currentIndex].properties.slideshowDelay;
		...
		*/
		function createNavigationThumbnails() {
			...
			for (var i = 0; i < slides.length; i++) {
				if (slides[i].properties.thumbnailType == 'navigation') {
					...
					if (self.settings.thumbnailOrientation == 'horizontal')
						thumbnail.css('left', numThumbnails * thumbnail.outerWidth(true));
					else
						thumbnail.css('top', numThumbnails * thumbnail.outerHeight(true));
			...
			totalThumbnailPages = Math.ceil(numThumbnails / self.settings.visibleThumbnails);
